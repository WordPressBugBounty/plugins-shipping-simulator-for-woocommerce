document.addEventListener('DOMContentLoaded', function () {
    ...
    function setFormDisabled(disabled = true) {
        ...
        if (disabled) {
            ...
            input.style.color = WcShippingSimulatorData.inputStyles.color || '#333';
            input.style.cursor = '';
            
            button.style.backgroundColor = WcShippingSimulatorData.buttonStyles.backgroundColor || '#0073aa';
            button.style.color = WcShippingSimulatorData.buttonStyles.color || '#fff';
            button.style.cursor = '';
        }
    }
    ...
    function setupVariationChangeListener() {
        ...
        if (variationForm) {
            ...
            variationForm.addEventListener('found_variation', function(event) {
                ...
                const inputEl = document.querySelector('.wc-shipping-simulator-input-current-style');
                const lastPostcode = inputEl ? formatCEP(inputEl.value) : '';
                if (WcShippingSimulatorData.enable_search === 'yes' && lastPostcode && isLastCepValid() !== false) {
                    setTimeout(() => {
                        const submitButton = document.querySelector('.wc-shipping-simulator-button-current-style');
    ...
    function checkAndUpdateFormState() {
        if (isVariableProduct()) {
            ...
            if (hasVariationSelected()) {
                ...
                if (WcShippingSimulatorData.enable_search === 'yes' && lastVariationId !== currentVariationId) {
                    ...
                    if (inputPostcode && inputPostcode.value && inputPostcode.value.trim().length >= 9) {
                        ...
                        const cachedData = getCachedShippingData(postcode, WcShippingSimulatorData.product_id);
                        if (cachedData) {
                            const infoBlock = document.querySelector('.wc-shipping-simulator-info-block');
                            const form = document.querySelector('#wc-shipping-simulator-postcode-form');
    ...
    function fetchProductNonce(callback) {
        ...
        formData.append('action_nonce', 'wc_shipping_simulator_register_product_address');
    ...
    function createInfoBlock(productInfo, shippingRates, postcode, form) {
        ...
        productIcon.src = WcShippingSimulatorData.details_icon.product;
        productIcon.alt = 'Produto';
        productIcon.classList.add('wc-shipping-simulator-icon');
        productIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        productName.classList.add('wc-shipping-simulator-product-name');
        if (font_class) {
        ...
        productQuantity.classList.add('wc-shipping-simulator-product-quantity');
        if (font_class) {
    ...
    function createForm() {
        ...
        // Lê CEP do campo de input
        const inputElCreate = document.querySelector('.wc-shipping-simulator-input-current-style');
        ...
        input.type = 'text';
        input.name = 'wc_shipping_simulator_custom_product_postcode';
        ...
        icon.src = WcShippingSimulatorData.icon // Define um ícone padrão da variável global
    ...
    function setPosition() {
        if (WcShippingSimulatorData.position === 'custom') {
            blockPosition = WcShippingSimulatorData.custom_position || 'h1[class*="title"]'; // Posição personalizada definida pelo usuário
        } else {
            const position = WcShippingSimulatorData.position || 'top'; // Posição padrão é 'top'
            if (position === 'middle') {
    ...
    const observer = new MutationObserver(function (mutationsList, observer) {
        mutationsList.forEach((mutation) => {
            if (mutation.type === 'childList') {
                ...
                const existingContainer = document.querySelector('.wc-shipping-simulator-parent-container');
                ...
                if (targetElement && !containerFound && !oldForm) {
                    ...
                    const inputObs = document.querySelector('.wc-shipping-simulator-input-current-style');
                    const lastPostcode = inputObs ? formatCEP(inputObs.value) : '';
                    let initializeData = {
                        product: {
                            name: '*******',
                            quantity: WcShippingSimulatorData.quantity,
                            currency_symbol: 'R$',
                    ...
                    if (lastPostcode) {
                        const cachedData = getCachedShippingData(lastPostcode, WcShippingSimulatorData.product_id);
                        if (cachedData) {
                            initializeData = {
                            ...
                            if (inputPostcode) {
                                ...
                                if (isVariableProduct()) {
                                    ...
                                    if (!hasVariationSelected()) {
                                        ...
                                        if (WcShippingSimulatorData.enable_search === 'yes') {
                                            const cachedData = getCachedShippingData(lastPostcode, WcShippingSimulatorData.product_id);
                                            if (cachedData) {
                                                const infoBlock = document.querySelector('.wc-shipping-simulator-info-block');
                                ...
                                if (WcShippingSimulatorData.enable_search && WcShippingSimulatorData.enable_search === 'yes') {
                                    // Verifica se existe cache para este CEP e produto específico
                                    const cachedData = getCachedShippingData(lastPostcode, WcShippingSimulatorData.product_id);
                                    ...
                                    if (cachedData) {
                                        ...
                                        if (WcShippingSimulatorData.enable_search === 'yes') {
                                            ...
                                            setTimeout(() => {
                                                const submitButton = form.querySelector('.wc-shipping-simulator-button-current-style');
                                                if (submitButton && !submitButton.disabled) {
                                                    submitButton.click();
                                                }
                                            }, 100);
        ...
        await fetch(apiUrl, {
            ...
            .then(data => {
                ...
                if (data.status === true) {
                    ...
                    const formData = new FormData();
                    formData.append('action', 'wc_shipping_simulator_register_product_address');
                    formData.append('product_id', WcShippingSimulatorData.product_id);
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (shippingRates.digital === true) {
                    ...
                    setCachedShippingData(postcode, WcShippingSimulatorData.product_id, digitalProductData);

                    // Tratamento para produtos digitais
                    const contentBlock = infoBlock.querySelector('.wc-shipping-simulator-content-block');
                    
                    const productName = infoBlock.querySelector('.wc-shipping-simulator-product-name');
                    if (productName) {
                ...
                setCachedShippingData(postcode, WcShippingSimulatorData.product_id, dataWithTimestamp);
                ...
                const productQuantity = infoBlock.querySelector('.wc-shipping-simulator-product-quantity');
                if (productQuantity) {
    ...
    function processShippingRatesFromCache(cachedData, form, infoBlock, postcode) {
        try {
            ...
            const cacheKey = variationId > 0 
                ? `${WcShippingSimulatorData.product_id}_${variationId}_qty${currentQuantity}`
                : `${WcShippingSimulatorData.product_id}_qty${currentQuantity}`;
            
            if (cache[postcode] && cache[postcode][cacheKey]) {
                delete cache[postcode][cacheKey];
                localStorage.setItem('wc_shipping_simulator_product_cache', JSON.stringify(cache));
            }
        }
    }

    function getProductCache() {
        const cacheKey = 'wc_shipping_simulator_product_cache';
    ...
    (function checkInitialRender() {
        ...
        if (targetElement && !containerFound && !oldForm && !existingContainer) {
            ...
            // Lê CEP do campo de input (preenchido por fetchUserPostcodeAndInitialize)
            const inputChk = document.querySelector('.wc-shipping-simulator-input-current-style');
            ...
            fetchProductNonce(function () {
                const inputChk2 = document.querySelector('.wc-shipping-simulator-input-current-style');
