document.addEventListener('DOMContentLoaded', function () {
    const WcShippingSimulatorData = window.WcShippingSimulatorData || {};

    let font_class = WcShippingSimulatorData.inputStyles.fontClass || '';
    ...
    function fetchUserPostcodeAndInitialize() {
        ...
        formData.append('action', 'wc_shipping_simulator_get_user_postcode');
        formData.append('nonce', WcShippingSimulatorData.get_postcode_nonce);

        // Adiciona timestamp na URL para evitar cache
        const ajaxUrlWithBuster = WcShippingSimulatorData.ajaxurl + '?t=' + Date.now();
    ...
    function processUserPostcode(cartCep) {
        ...
        if (normalizedCartCep && cepRegex.test(normalizedCartCep)) {
            ...
            const input = document.querySelector('.wc-shipping-simulator-input-current-style');
            if (input) {
                applyFormatToInput(input, normalizedCartCep);
            }
            
            // Se auto-consulta está habilitada E a flag não indica falha anterior
            if (WcShippingSimulatorData.enable_search === 'yes' && isLastCepValid() !== false) {
                setTimeout(() => {
                    const button = document.querySelector('.wc-shipping-simulator-button-current-style');
    ...
    function createParentContainer() {
        ...
        parentContainer.classList.add('wc-shipping-simulator-parent-container');
    ...
    function fetchCartNonce(callback) {
        const formData = new FormData();
        formData.append('action', 'wc_shipping_simulator_get_nonce');
        formData.append('action_nonce', 'wc_shipping_simulator_register_cart_address');

        fetch(WcShippingSimulatorData.ajaxurl + '?t=' + Date.now(), {
            method: 'POST',
            body: formData
        })
            .then(response => response.json())
    ...
    function enablePostcodeForm() {
        ...
        if (button) {
            ...
            button.style.backgroundColor = WcShippingSimulatorData.buttonStyles.backgroundColor || '#0073aa';
            button.style.cursor = '';
        }
        if (input) {
            input.disabled = false;
            input.style.backgroundColor = WcShippingSimulatorData.inputStyles.backgroundColor || '#fff';
            input.style.cursor = '';
        }

        const cepBlock = document.querySelector('.wc-shipping-simulator-current-postcode-block');
        if (cepBlock) {
            cepBlock.style.display = 'flex';
        }

        const updateIcon = document.querySelector('.wc-shipping-simulator-update-icon');
        const updateIconContainer = document.querySelector('.wc-shipping-simulator-update-icon-container');
    ...
    function createCurrentPostcodeBlock(postcode, form) {
        ...
        currentPostcodeBlock.classList.add('wc-shipping-simulator-current-postcode-block');

        const toggleAndPostcodeWrapper = document.createElement('div');
        toggleAndPostcodeWrapper.classList.add('wc-shipping-simulator-toggle-postcode-wrapper');

        const toggleButton = document.createElement('button');
        toggleButton.type = 'button';
        displayButton(toggleButton, 'up', 'Esconder detalhes de entrega');
        toggleButton.classList.add('wc-shipping-simulator-toggle-button');

        const postcodeText = document.createElement('span');
        postcodeText.innerHTML = `<strong>CEP</strong>: ${postcode}`;
        postcodeText.classList.add('wc-shipping-simulator-current-postcode-text');
        if (font_class) {
            postcodeText.classList.add(font_class);
        }

        toggleButton.addEventListener('click', () => {
            const contentBlock = document.querySelector('.wc-shipping-simulator-content-block');
            if (contentBlock) {
        ...
        changeButton.classList.add('wc-shipping-simulator-change-postcode-button');
        if (font_class) {
            changeButton.classList.add(font_class);
        }

        changeButton.addEventListener('click', () => {
            const infoBlock = document.querySelector('.wc-shipping-simulator-info-block');
            if (infoBlock) {
                infoBlock.style.display = 'none';
                
                // Reseta o estado do contentBlock para garantir expansão correta na próxima consulta
                const contentBlock = infoBlock.querySelector('.wc-shipping-simulator-content-block');
                if (contentBlock) {
                    contentBlock.classList.remove('expanded');
                ...
                const toggleButton = infoBlock.querySelector('.wc-shipping-simulator-toggle-button');
                if (toggleButton) {
                    toggleButton.innerHTML = '';
                    displayButton(toggleButton, 'up', 'Esconder detalhes de entrega');
                }
    ...
    function createDynamicStyles() {
        ...
        const originalColor = WcShippingSimulatorData.inputStyles.backgroundColor || '#ffffff';
        const darkerColor = darkenColor(originalColor, 10);
        const iconColor = WcShippingSimulatorData.iconColor || 'blue-icon';
        ...
        const css = `
            .wc-shipping-simulator-info-block {
                color: ${WcShippingSimulatorData.inputStyles.color} !important;
                border-radius: ${WcShippingSimulatorData.inputStyles.borderRadius} !important;
                padding: 0px !important;
                margin: 20px 0px !important;
                font-size: 14px !important;
            }

            .wc-shipping-simulator-current-postcode-block {
                display: flex;
                align-items: center;
            ...
            .wc-shipping-simulator-content-block {
                margin-top: -3px;
                padding: 0px 20px;
            ...
            .wc-shipping-simulator-content-block.expanded {
                height: auto; 
                padding: 10px 20px;
                border-bottom-right-radius: ${WcShippingSimulatorData.inputStyles.borderRadius} !important;
                border-bottom-left-radius: ${WcShippingSimulatorData.inputStyles.borderRadius} !important;
                border: ${WcShippingSimulatorData.inputStyles.borderWidth} ${WcShippingSimulatorData.inputStyles.borderStyle} ${WcShippingSimulatorData.inputStyles.borderColor} !important;
                border-top: 0px !important;
            }

            .wc-shipping-simulator-separator {
                border: none;
            ...
            .wc-shipping-simulator-update-section {
                display: flex;
                align-items: center;
                gap: 12px;
                margin-top: 10px;
            ...
            .wc-shipping-simulator-update-icon-container {
            ...
            .wc-shipping-simulator-update-icon-container:focus {
                outline: none !important;
                box-shadow: none !important;
            }

            .wc-shipping-simulator-update-icon-container:hover {
                background-color: ${themeColor}1a !important;
            }

            .wc-shipping-simulator-update-icon-container:hover .wc-shipping-simulator-update-icon {
                opacity: 1;
                transform: rotate(180deg);
            }

            .wc-shipping-simulator-update-icon {
                width: 32px;
                height: 32px;
                opacity: 0.8;
            ...
            .wc-shipping-simulator-update-icon.spinning {
                animation: wc-shipping-simulator-spin 1s linear infinite;
                opacity: 1;
            }

            .wc-shipping-simulator-update-icon-container.spinning-container {
                opacity: 0.8 !important;
                cursor: not-allowed !important;
            }

            .wc-shipping-simulator-update-icon-container.spinning-container:hover {
                background-color: transparent !important;
            }

            .wc-shipping-simulator-update-icon-container.spinning-container:hover .wc-shipping-simulator-update-icon {
                transform: none !important;
            }

            @keyframes wc-shipping-simulator-spin {
                from { transform: rotate(0deg); }
                to { transform: rotate(360deg); }
            }

            .wc-shipping-simulator-update-text-container {
                flex: 1;
                line-height: 1.4;
            }

            .wc-shipping-simulator-update-date {
                width: fit-content;
                padding: 3px;
                font-size: 13px;
                font-weight: 600;
                margin: 0 0 4px 0 !important;
                color: ${WcShippingSimulatorData.inputStyles.color || '#333'};
                opacity: 0.8;
                transition: all 0.3s ease;
            }

            .wc-shipping-simulator-update-date.flash {
                animation: wc-shipping-simulator-flash 2s ease-in-out;
            }

            @keyframes wc-shipping-simulator-flash {
                0%, 100% { 
            ...
            .wc-shipping-simulator-info-text {
                font-size: 12px;
                padding: 3px;
                margin: 0;
                color: ${WcShippingSimulatorData.inputStyles.color || '#333'};
    ...
    function displayButton(component, name, text) {
        ...
        toggleIcon.src = WcShippingSimulatorData.display_icon[name];
        toggleIcon.alt = text;
        toggleIcon.classList.add('wc-shipping-simulator-toggle-icon');
        toggleIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        component.appendChild(toggleIcon);
    }

    function createInfoBlock(cartInfo, shippingRates, postcode, form) {
        const infoBlock = document.createElement('div');
        infoBlock.classList.add('wc-shipping-simulator-info-block');
        infoBlock.classList.add(font_class);

        // Lê CEP do campo de input em vez de cache
        const inputElCB = document.querySelector('.wc-shipping-simulator-input-current-style');
        const lastPostcode = inputElCB ? formatCEP(inputElCB.value) : '';
        const hasRealData = cartInfo && cartInfo.name && cartInfo.name !== '****';

        if (!lastPostcode || WcShippingSimulatorData.enable_search !== 'yes') {
            infoBlock.style.display = 'none';
        } else if (hasRealData) {
        ...
        contentBlock.classList.add('wc-shipping-simulator-content-block');
        ...
        cartIcon.src = WcShippingSimulatorData.details_icon.cart;
        cartIcon.alt = 'Produto';
        cartIcon.classList.add('wc-shipping-simulator-icon');
        cartIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        cartName.classList.add('wc-shipping-simulator-cart-name');
        if (font_class) {
        ...
        quantityIcon.src = WcShippingSimulatorData.details_icon.quantity;
        quantityIcon.alt = 'Quantidade';
        quantityIcon.classList.add('wc-shipping-simulator-icon');
        quantityIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        cartQuantity.classList.add('wc-shipping-simulator-cart-quantity');
        if (font_class) {
            cartQuantity.classList.add(font_class);
        }

        const shippingMethods = document.createElement('div');
        shippingMethods.classList.add('wc-shipping-simulator-shipping-methods');

        const shippingTitle = document.createElement('p');

        const shippingIcon = document.createElement('img');
        shippingIcon.src = WcShippingSimulatorData.icon;
        shippingIcon.alt = 'Entrega';
        shippingIcon.classList.add('wc-shipping-simulator-icon');
        shippingIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        shippingList.classList.add('wc-shipping-simulator-shipping-list');
        ...
        separator.classList.add('wc-shipping-simulator-separator');

        const updateSection = document.createElement('div');
        updateSection.classList.add('wc-shipping-simulator-update-section');

        const iconContainer = document.createElement('button');
        iconContainer.type = 'button';
        iconContainer.classList.add('wc-shipping-simulator-update-icon-container');
        iconContainer.title = 'Clique para atualizar os dados de frete do carrinho';

        const updateIcon = document.createElement('img');
        updateIcon.src = WcShippingSimulatorData.update_icon.updates;
        updateIcon.alt = 'Atualizado';
        updateIcon.classList.add('wc-shipping-simulator-update-icon');
        updateIcon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        iconContainer.addEventListener('click', function () {
            ...
            const inputEl2 = document.querySelector('.wc-shipping-simulator-input-current-style');
        ...
        textContainer.classList.add('wc-shipping-simulator-update-text-container');

        const updateDate = document.createElement('p');
        updateDate.classList.add('wc-shipping-simulator-update-date');
        const currentDate = new Date().toLocaleString('pt-BR');
        updateDate.textContent = `Atualizado em ${currentDate}`;

        const infoText = document.createElement('p');
        infoText.classList.add('wc-shipping-simulator-info-text');
    ...
    function createForm() {
        ...
        form.id = 'wc-shipping-simulator-postcode-form';
        form.style.marginTop = '20px';
        form.style.padding = '0px';

        // Lê CEP do campo de input
        const inputElCf = document.querySelector('.wc-shipping-simulator-input-current-style');
        const lastPostcode = inputElCf ? formatCEP(inputElCf.value) : (ajaxFetchedPostcode || '');
        if (lastPostcode && (WcShippingSimulatorData.enable_search === 'yes' || hasUserMadeQuery)) {
        ...
        input.type = 'text';
        input.name = 'wc_shipping_simulator_custom_cart_postcode';
        input.placeholder = WcShippingSimulatorData.placeholder || 'Digite o CEP';
        input.classList.add('wc-shipping-simulator-input-current-style');
        if (font_class) {
        ...
        const inputStyles = WcShippingSimulatorData.inputStyles || {};
        ...
        icon.src = WcShippingSimulatorData.icon
        icon.alt = 'Ícone de entrega';
        icon.classList.add('wc-shipping-simulator-icon-current-style');
        icon.classList.add(WcShippingSimulatorData.iconColor || 'black-icon');
        ...
        const buttonStyles = WcShippingSimulatorData.buttonStyles || {};
    ...
    function setPosition() {
        ...
        const isBlocksCart = WcShippingSimulatorData.is_blocks_cart || false;
        ...
        if (WcShippingSimulatorData.position === 'custom') {
            blockPosition = WcShippingSimulatorData.custom_position || (isBlocksCart ? 'h2[class*="order"]' : '.woocommerce-notices-wrapper');
        } else {
            const position = WcShippingSimulatorData.position || 'top';
            if (isBlocksCart) {
    ...
    function updateCepComponentAfterCartChange() {
        ...
        updateTimeout = setTimeout(() => {
            ...
            const infoBlock = document.querySelector('.wc-shipping-simulator-info-block');
            const form = document.querySelector('#wc-shipping-simulator-postcode-form');
            ...
            if (WcShippingSimulatorData.enable_search !== 'yes' && hasUserMadeQuery) {
            ...
            if (lastPostcode && infoBlock && form) {
                ...
                const input = form.querySelector('.wc-shipping-simulator-input-current-style');
                if (input) {
                    applyFormatToInput(input, lastPostcode);
                }
                ...
                setTimeout(() => {
                    ...
                    if (lastPostcode && WcShippingSimulatorData.enable_search === 'yes' && isLastCepValid() !== false) {
                        ...
                        if (cachedData) {
                            ...
                            if (wasComponentExpanded) {
                                ...
                                const updateButton = infoBlock.querySelector('.wc-shipping-simulator-update-icon-container');
                                ...
                                if (updateButton) {
                                    ...
                                    const button = form.querySelector('.wc-shipping-simulator-button-current-style');
                                    if (button && !button.disabled) {
                                        button.disabled = false;
                                        button.click();
                                    }
    ...
    function updateTimestamp(infoBlock, addFlashAnimation = false) {
        const updateDate = infoBlock.querySelector('.wc-shipping-simulator-update-date');
        if (updateDate) {
            const currentDate = new Date().toLocaleString('pt-BR');
            updateDate.textContent = `Atualizado em ${currentDate}`;
        ...
        setTimeout(() => {
            if (isBlocksCart) {
                ...
                const updateEvent = new CustomEvent('wc-blocks-cart-update', {
                    detail: { source: 'wc-shipping-simulator-cep-plugin' },
                    bubbles: true
                });
    ...
    async function sendCEP(postcode, forceRequest = false) {
        ...
        if (infoBlock && isComponentCurrentlyVisible) {
            const shippingList = infoBlock.querySelector('.wc-shipping-simulator-shipping-list');
            if (shippingList) {
        ...
        if (typeof wpApiSettings !== 'undefined' && wpApiSettings.root) {
            apiUrl = wpApiSettings.root + `wc-shipping-simulator/v1/cep/?postcode=${postcode}&t=${timestamp}`;
        } else {
            if (typeof WcShippingSimulatorData !== 'undefined' && WcShippingSimulatorData.wooUrl !== '') {
                apiUrl = WcShippingSimulatorData.wooUrl + `/wp-json/wc-shipping-simulator/v1/cep/?postcode=${postcode}&t=${timestamp}`;
            } else {
                apiUrl = `/wp-json/wc-shipping-simulator/v1/cep/?postcode=${postcode}&t=${timestamp}`;
            }
        }
        ...
        await fetch(apiUrl, {
            ...
            .then(data => {
                ...
                if (data.status === true) {
                    ...
                    const formData = new FormData();
                    formData.append('action', 'wc_shipping_simulator_register_cart_address');
                    ...
                    fetch(addressAPIUrl, {
                        ...
                        .then(response => {
                            if (response.success && response.data && response.data.digital) {
                                ...
                                const cartQuantity = infoBlock.querySelector('.wc-shipping-simulator-cart-quantity');
                                if (cartQuantity) {
                                ...
                                if (infoBlock) {
                                    const postcodeText = infoBlock.querySelector('.wc-shipping-simulator-current-postcode-text');
                                    const shippingList = infoBlock.querySelector('.wc-shipping-simulator-shipping-list');
                                    ...
                                    const updateIcon = infoBlock.querySelector('.wc-shipping-simulator-update-icon');
                                    const updateIconContainer = infoBlock.querySelector('.wc-shipping-simulator-update-icon-container');
                                    if (updateIcon && updateIcon.classList.contains('spinning')) {
                                        setTimeout(() => {
                                            updateIcon.classList.remove('spinning');
                                            if (updateIconContainer) {
    ...
    function processShippingRates(response, form, infoBlock, postcode, cartDataAtRequestTime = null) {
        return new Promise((resolve, reject) => {
            try {
                ...
                let contentBlock = infoBlock.querySelector('.wc-shipping-simulator-content-block');

                // Remove mensagem de erro anterior, se existir
                if (contentBlock) {
                    const oldError = contentBlock.querySelector('.wc-shipping-simulator-error-message');
                    if (oldError) oldError.remove();
                }

                if (!shippingRates || !Array.isArray(shippingRates.shipping_rates) || shippingRates.shipping_rates.length === 0) {
                    setLastCepValid(false);
                    // Esconde todos os componentes filhos, exceto .wc-shipping-simulator-update-section
                    if (contentBlock) {
                        ...
                        const currentPostcodeText = infoBlock.querySelector('.wc-shipping-simulator-current-postcode-text');
                        if (currentPostcodeText) {
                            currentPostcodeText.innerHTML = `<strong>CEP</strong>: ${postcode}`;
                        }
                        // Adiciona mensagem de erro
                        let errorMsg = contentBlock.querySelector('.wc-shipping-simulator-error-message');
                        if (!errorMsg) {
                            errorMsg = document.createElement('p');
                            errorMsg.className = 'wc-shipping-simulator-error-message';
                ...
                if (contentBlock) {
                    const errorMessage = contentBlock.querySelector('.wc-shipping-simulator-error-message');
                    if (errorMessage) {
                        errorMessage.remove();
                    }
                    Array.from(contentBlock.children).forEach(child => {
                        if (child.classList.contains('wc-shipping-simulator-update-section')) {
                ...
                const shippingList = contentBlock.querySelector('.wc-shipping-simulator-shipping-list');
    ...
    function processShippingRatesFromCache(cachedData, form, infoBlock, postcode) {
        try {
            ...
            const contentInfoBlock = infoBlock.querySelector('.wc-shipping-simulator-content-block');
            ...
            if (cache[postcode]) {
                ...
                localStorage.setItem('wc_shipping_simulator_cart_cache', JSON.stringify(cache));
            }
        }
    }

    function getCartCache() {
        const cacheKey = 'wc_shipping_simulator_cart_cache';
        ...
        if (cachedData) {
            try {
                ...
                const cacheTimeMinutes = parseInt(WcShippingSimulatorData.cache_time) || 0;
    ...
    function getCachedCartShippingData(postcode) {
        ...
        if (cache[postcode]) {
            ...
            if (isCartConfigurationEqual(currentCartData, cachedData.cart_data)) {
                ...
                const lastAutoClick = sessionStorage.getItem('wc_shipping_simulator_last_auto_click');
                const now = Date.now();
                ...
                if (canAutoClick && !isSendingCEP) {
                    setTimeout(() => {
                        ...
                        if (button && !button.disabled && !isSendingCEP) {
                            sessionStorage.setItem('wc_shipping_simulator_last_auto_click', now.toString());
                            button.click();
                        }
                    }, 100);
                }
    ...
    function isLastCepValid() {
        const key = 'wc_shipping_simulator_last_cep_valid';
    ...
    function setLastCepValid(valid) {
        localStorage.setItem('wc_shipping_simulator_last_cep_valid', valid ? 'true' : 'false');
    }
    ...
    function persistCartPostcodeOnly(postcode) {
        ...
        const formData = new FormData();
        formData.append('action', 'wc_shipping_simulator_persist_postcode');
        formData.append('nonce', WcShippingSimulatorData.get_postcode_nonce);
        formData.append('postcode', postcode);

        const apiUrl = WcShippingSimulatorData.ajaxurl + '?t=' + Date.now();
        fetch(apiUrl, {
            method: 'POST',
            body: formData,
        }).catch(() => {});
    ...
    function getCurrentCartData() {
        ...
        if (cartData.length === 0) {
            ...
            } else if (WcShippingSimulatorData.quantity) {
                // Usa dados do WcShippingSimulatorData como último recurso
                const quantity = parseInt(WcShippingSimulatorData.quantity) || 1;
                cartData.push({
                    id: 'unknown',
                    quantity: quantity,
                    variation_id: 0
    ...
    function isTokenValid() {
        const currentToken = WcShippingSimulatorData.cache_token || '';
    ...
    function updateTokenCache() {
        ...
        const tokenCacheKey = 'wc_shipping_simulator_token_cache_data';
    ...
    function getTokenCacheData() {
        ...
        localStorage.removeItem('wc_shipping_simulator_cart_cache');
        localStorage.removeItem('wc_shipping_simulator_product_cache');
        localStorage.removeItem('wc_shipping_simulator_token_cache_data');
        localStorage.removeItem('wc_shipping_simulator_last_cep_valid');
        // Remove também caches antigos para limpeza
        localStorage.removeItem('wc_shipping_simulator_token_cache');
        localStorage.removeItem('wc_shipping_simulator_cart_postcode_cache_simple');
        localStorage.removeItem('wc_shipping_simulator_last_postcode');
        localStorage.removeItem('wc_shipping_simulator_postcode_cache');
    }
    ...
    function startComponentMonitor() {
        ...
        componentCheckInterval = setInterval(() => {
            const existingComponent = document.querySelector('.wc-shipping-simulator-parent-container');
    ...
    function createComponentIfNeeded() {
        ...
        if (targetElement && !existingComponent && !containerFound) {
            ...
            const inputChkObs = document.querySelector('.wc-shipping-simulator-input-current-style');
            ...
            let initializeData = {
                cart: {
                    name: '****',
                    quantity: WcShippingSimulatorData.quantity || 1,
                    currency_symbol: 'R$',
                    currency_minor_unit: 2,
                },
                shipping_rates: [
            ...
            fetchCartNonce(function () {
                const inputNonce = document.querySelector('.wc-shipping-simulator-input-current-style');
                const lastPostcode = inputNonce ? formatCEP(inputNonce.value) : '';

                if (lastPostcode) {
                    const inputPostcode = document.querySelector('.wc-shipping-simulator-input-current-style');
                    if (inputPostcode) {
                        inputPostcode.value = lastPostcode;

                        // Se a consulta automática está habilitada OU o usuário já fez uma consulta anterior
                        const shouldRestoreFromCache = (WcShippingSimulatorData.enable_search && WcShippingSimulatorData.enable_search === 'yes') || hasUserMadeQuery;
                        ...
                        if (shouldRestoreFromCache) {
                            ...
                            if (cachedData) {
                                ...
                                setTimeout(() => {
                                    const button = form.querySelector('.wc-shipping-simulator-button-current-style');
                                    if (button && WcShippingSimulatorData.enable_search === 'yes') {
                                        button.click();
                                    }
                                }, 300);
                            }
