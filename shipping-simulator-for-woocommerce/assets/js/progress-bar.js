(function ($) {
	...
	const progressConfig = typeof wcShippingSimulatorProgress !== 'undefined' ? wcShippingSimulatorProgress : {};
	...
	let minValue = typeof wcShippingSimulatorProgress !== 'undefined'
		? parseFloat(wcShippingSimulatorProgress.min_free_shipping_value)
		: 0;
...
// Função removida - não vamos mais calcular valores iniciais com data do PHP
	...
	function getCartShippingData() {
		...
		cartUpdateTimeout = setTimeout(() => {
			...
			formData.append('action', 'wc_shipping_simulator_get_cart_shipping_status');
	...
	function insertOrUpdateProgressBar() {
		...
		let progressBar = document.querySelector('.wc-shipping-simulator-progress-bar');
		if (!progressBar) {
			let progressBarContainer = document.createElement('div');
			progressBarContainer.className = 'wc-shipping-simulator-progress-bar';
			...
			progressBar.className = 'wc-shipping-simulator-progress';
			...
			if (enableProgressBarValue) {
				...
				progressBarInnerText.className = 'wc-shipping-simulator-progress-inner-text';
			...
			progressBarText.className = 'wc-shipping-simulator-progress-text';
			...
			if (!document.getElementById('wc-shipping-simulator-progress-styles')) {
				const style = document.createElement('style');
				style.id = 'wc-shipping-simulator-progress-styles';
				style.textContent = `
					...
					.wc-shipping-simulator-progress.loading {
			...
			if (previousPorcent !== percent || isLoading) {
				let bar = progressBar.querySelector('.wc-shipping-simulator-progress');
				if (bar) {
					if (isLoading) {
				...
				let innerText = progressBar.querySelector('.wc-shipping-simulator-progress-inner-text');
				if (enableProgressBarValue) {
					if (innerText) {
						innerText.textContent = barText;
					} else {
						// Se não existe mas deveria existir, cria o elemento
						let progressBarWrapper = progressBar.querySelector('.wc-shipping-simulator-progress').parentNode;
						let newInnerText = document.createElement('div');
						newInnerText.className = 'wc-shipping-simulator-progress-inner-text';
				...
				let text = progressBar.querySelector('.wc-shipping-simulator-progress-text');
				if (text) {
	...
	function init() {
		...
		$(document).on('wc-shipping-simulator-cart-update-start', function () {
			startLoadingState();
